...
forms returns [Form result]
				: 'form' Ident '{' s=statements '}' {$result = new Form(new Ident($Ident.text), (StatementList)$s.result);}
                ;


statements returns [StatementList result]
	: s1=statement {$result = new StatementList();
	      $result.addStatementtoList($s1.result);}
	| ss=statements s2=statement {$result = $ss.result;
		  $ss.result.addStatementtoList($s2.result);
	}
	;									
...
question returns [Statement result] 
     : Ident ':' Str v1=valuetype  {$result = new Question(new Ident($Ident.text), new StrLiteral($Str.text), $v1.result);}                                        
     | Ident ':' Str v2=valuetype'(' e1= orExpr ')'  {$result = new Computedquest (new Ident($Ident.text), new StrLiteral($Str.text), $v2.result, $e1.result);}
     ;
     
ifstatement returns [Statement result]
        : 'if' '(' expr=orExpr ')' '{'  s=statements '}' {$result = new Ifstate($s.result, $expr.result);}
        ;
          
ifelsestate returns [Statement result]
         : 'if' '(' expr=orExpr ')' '{'  s1=statements '}'  'else' '{'  s2=statements  '}' {$result = new IfElse($s1.result, $expr.result, $s2.result);}
         ;  
