...
@header {
  ...
  import ql.ast.node.arithmetic.*;
  import ql.ast.node.conditional.*;
  import ql.ast.node.relational.*;
  import ql.ast.node.sign.*;
  import ql.ast.node.syntactic.*;
  import ql.ast.node.types.*;
  import ql.ast.checker.*;
...
question returns [Question result]
  : IDENT ':' string questiontype {
    $result = new Question (new Ident($IDENT.text),
     new Label($string.result), $questiontype.result); 
    } 
  ('('expression')' {
    $result = new QuestionWithExpression (new Ident($IDENT.text),new Label($string.result), 
...
primary returns [Expression result]
  ...
  | string {$result = new Str($string.result);}
  | '(' x=orExpr ')'{ $result = $x.result; }            
  ;
...
string returns [String result]
    : s=STRING {$result=new String(($s.text).substring(1,($s.text).length()-1));}
    ;
